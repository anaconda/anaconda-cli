...
mod tests {
    ...
    mod unix_tests {
        ...
        #[test]
        fn test_create_bin_symlink_creates_symlink() {
            let temp = TempDir::new().unwrap();
            let bin_dir = temp.path().join("bin");
            let prefix = temp.path().join("tool-prefix");
            std::fs::create_dir_all(&bin_dir).unwrap();
            std::fs::create_dir_all(prefix.join("bin")).unwrap();

            // Create fake tool binary
            let tool_bin = prefix.join("bin").join("mytool");
            std::fs::write(&tool_bin, "#!/bin/bash\necho hello").unwrap();

            let binary = PathBuf::from("bin/mytool");
            let result = create_bin_symlink(&bin_dir, &prefix, &binary);
            assert!(result.is_ok(), "create_bin_symlink failed: {:?}", result);

            // Check symlink was created
            let symlink_path = bin_dir.join("mytool");
            ...
            assert_eq!(target, tool_bin);
        }

        #[test]
        fn test_create_bin_symlink_replaces_existing_symlink() {
            let temp = TempDir::new().unwrap();
            let bin_dir = temp.path().join("bin");
            let prefix = temp.path().join("tool-prefix");
            std::fs::create_dir_all(&bin_dir).unwrap();
            ...
            // Create existing symlink to a different target
            let symlink_path = bin_dir.join("mytool");
            ...
            // Check symlink now points to new target
            let target = std::fs::read_link(&symlink_path).unwrap();
            assert_eq!(target, tool_bin);
        }

        #[test]
        fn test_create_bin_symlink_skips_missing_binary() {
            let temp = TempDir::new().unwrap();
            let bin_dir = temp.path().join("bin");
            let prefix = temp.path().join("tool-prefix");
            std::fs::create_dir_all(&bin_dir).unwrap();
            ...
            let binary = PathBuf::from("bin/nonexistent");
            let result = create_bin_symlink(&bin_dir, &prefix, &binary);
            assert!(result.is_ok(), "should succeed with warning");

            // No symlink should be created
            assert!(!bin_dir.join("nonexistent").exists());
        }

        #[test]
        fn test_create_bin_symlinks_multiple() {
            let temp = TempDir::new().unwrap();
            let prefix = temp.path().join("tool-prefix");
            std::fs::create_dir_all(prefix.join("bin")).unwrap();

            // Create fake tool binaries
            std::fs::write(prefix.join("bin").join("tool1"), "binary1").unwrap();
            std::fs::write(prefix.join("bin").join("tool2"), "binary2").unwrap();

            temp_env::with_var("ANA_HOME", Some(temp.path().to_str().unwrap()), || {
                let binaries = vec![PathBuf::from("bin/tool1"), PathBuf::from("bin/tool2")];
                let result = create_bin_symlinks(&prefix, &binaries);
                assert!(result.is_ok());

                let bin_dir = paths::bin_dir();
                assert!(bin_dir.join("tool1").is_symlink());
                assert!(bin_dir.join("tool2").is_symlink());
            });
        }
    }
