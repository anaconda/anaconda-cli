...
Since `ana org` is gated behind login, invocations run with a completed
login against the mock auth server (run_ana_logged_in).
"""
...
class TestOrg:
    ...
    def test_org_help_auto_installs_and_forwards(
        ...
        result = run_ana_logged_in("org", "--help")
    ...
    def test_org_short_help_is_forwarded(self, run_ana_logged_in: AnaRunner) -> None:
